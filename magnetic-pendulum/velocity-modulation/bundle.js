...
function acceleration(_ref) {
  var position = _ref.position;
  var time = _ref.time;
  return force({ position: position, time: time }) / mass;
}
...
function log() {
  ...
  var velocityChange = state.velocityChange;
  past.push({ time: time, position: position, velocityChange: velocityChange });
  if (past.length > 100) past.shift();
}

var forceArea = d3.svg.area().x(function (d) {
  return d.time - timeShift();
}).y1(function (d) {
  return force(d);
}).interpolate("monotone");

var velocityArea = d3.svg.area().x(function (d) {
  return d.time - timeShift();
}).y1(function (d) {
  return d.velocityChange * 10;
}).interpolate("monotone");
...
function update() {
  ...
  d3.select("#accelerate-right").attr("opacity", force);
  d3.select("#accelerate-left").attr("opacity", -force);

  d3.select("#bob").attr("cx", position).attr("opacity", opacity(state));

  d3.select("#force-area").datum(past).attr("d", forceArea);

  d3.select("#velocity-area").datum(past).attr("d", velocityArea);
}
